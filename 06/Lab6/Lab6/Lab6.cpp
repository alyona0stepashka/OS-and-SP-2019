...
void CALLBACK a_writeFile(DWORD p)
{
	...
	out.open("D:\\Study\\3k2s\\GitRep\\OS-and-SP-2019\\06\\Lab6\\Lab6\\hello.txt"); // окрываем файл для записи
	if (out.is_open())
	{
		out << buf << std::endl;
	}
...
void CALLBACK a_readFile(DWORD p)
{
	...
	std::string line;
	EnterCriticalSection(&cs);
	std::ifstream in("D:\\Study\\3k2s\\GitRep\\OS-and-SP-2019\\06\\Lab6\\Lab6\\hello.txt"); // окрываем файл для чтения
	if (in.is_open())
	{
...
{
	DWORD* zz = (DWORD*)ptr;
	std::cout << "Tread says: initial count is " << *zz << std::endl;
	SleepEx(INFINITE, TRUE);  // call async. proc
	std::cout << "Tread says: Final count from async proc is " << *zz << std::endl;
	getchar();
	return 0;
...
{
	std::cout << "Tread file says: START"<< std::endl;
	SleepEx(INFINITE, TRUE);  // call async. proc
	std::cout << "Tread file says: END!" << std::endl;
	getchar();
	return 0;
}
