// lab_05.cpp : Определяет экспортированные функции для приложения DLL.
//
...
const CLSID CLSID_XLApplication = { 0x00024500,0x0000,0x0000,{0xC0,0x00,0x00,0x00,0x00,0x00,0x00,0x46} }; // CLSID of Excel записывается в реестр
const IID   IID_Application = { 0x000208D5,0x0000,0x0000,{0xC0,0x00,0x00,0x00,0x00,0x00,0x00,0x46} }; // IID of _Application 

extern "C++" __declspec(dllexport) void Call_excell()
{
							...
							if (SUCCEEDED(hr))
							{
								...
								if (SUCCEEDED(hr))
								{
									...
									if (SUCCEEDED(hr))
									{
										...
										if (SUCCEEDED(hr))
										{
											...
											//	hr = pXLRange2->GetIDsOfNames(IID_NULL, &szInterior, 1, GetUserDefaultLCID(), &dispid);
											//	if (SUCCEEDED(hr))
											//	{
											//		IDispatch* pXLInterior = NULL;
											//		VariantInit(&vResult);
											//		hr = pXLRange2->Invoke(dispid, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_PROPERTYGET, &NoArgs, &vResult, NULL, NULL);
											//		if (SUCCEEDED(hr))
											//		{
											//			pXLInterior = vResult.pdispVal;
											...
											//				VariantClear(&CallArgs[0]);
											//				CallArgs[0].vt = VT_BSTR;
											//				CallArgs[0].bstrVal = SysAllocString(L"Consolas");
											//				DispParams.rgvarg = CallArgs;
											//				DispParams.rgdispidNamedArgs = &dispidNamed;
											//				DispParams.cArgs = 1;
											//				DispParams.cNamedArgs = 1;
								...
								if (SUCCEEDED(hr))
								{
									...
									if (SUCCEEDED(hr))
									{
										...
										VariantInit(&vResult);
										CallArgs[0].vt = VT_BSTR,
											CallArgs[0].bstrVal = SysAllocString(L"B5");
										DispParams.rgvarg = CallArgs;
										DispParams.rgdispidNamedArgs = 0;
										DispParams.cArgs = 1;  // Try to get Range
										DispParams.cNamedArgs = 0;  // Invoke _Worksheet::Range("A1")  << returns IDispatch** to dispinterface Range
										...
										if (SUCCEEDED(hr))
										{
											...
											if (SUCCEEDED(hr))
											{
												VariantClear(&CallArgs[0]);
												CallArgs[0].vt = VT_BSTR;
												CallArgs[0].bstrVal = SysAllocString(L"World");  //Try to set data to cell A1 using pXLRange
												DispParams.rgvarg = CallArgs;
												DispParams.rgdispidNamedArgs = &dispidNamed;
												DispParams.cArgs = 1;  // Try to write to Value member of Range dispinterface
												DispParams.cNamedArgs = 1;
