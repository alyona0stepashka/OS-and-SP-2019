					...
					if (SUCCEEDED(hr))
					{
						...
						if (SUCCEEDED(hr))
						{
							...
							if (SUCCEEDED(hr))
							{
								...
								IDispatch* pXLSheet1 = NULL;  // Try To Get ActiveSheet
								...
								if (SUCCEEDED(hr))
								{
									...
									if (SUCCEEDED(hr))
									{
										...
										CallArgs[0].bstrVal = SysAllocString(L"B3");
										DispParams.rgvarg = CallArgs;
										...
										if (SUCCEEDED(hr))
										{
											...
											if (SUCCEEDED(hr))
											{
												...
												CallArgs[0].vt = VT_BSTR;
												CallArgs[0].bstrVal = SysAllocString(L"Hellow");
												DispParams.rgvarg = CallArgs;
												DispParams.rgdispidNamedArgs = &dispidNamed;
												DispParams.cArgs = 1; 
												DispParams.cNamedArgs = 1;
											...
											if (SUCCEEDED(hr))
											{
												...
												if (SUCCEEDED(hr))
												{
													...
													if (SUCCEEDED(hr))
													{
														VariantClear(&CallArgs[0]);
														CallArgs[0].vt = VT_BSTR;
														CallArgs[0].bstrVal = SysAllocString(L"Wide Latin");
														DispParams.rgvarg = CallArgs;
														DispParams.rgdispidNamedArgs = &dispidNamed;
														DispParams.cArgs = 1;
														DispParams.cNamedArgs = 1;
